...
@Component({
  selector: 'app-game-sum-score',
  templateUrl: './game-sum-score.component.html',
  styleUrls: ['./game-sum-score.component.css']
})

export class GameSumScoreComponent implements OnInit {
  ...
  public haveGame:any =0;

  start_page_game_score(){
    this.todoServcie.getDateGameSumScore_played(this.member_id).subscribe((response1)=>{
       ...
       this.todoServcie.getHandicap_game_score(this.member_id).subscribe((response)=>{
          if(response.success!=false&&response.success!=null){
            ...
            for(var i=0;i<response1.data.length;i++){
             ...
             // debugger;
              if(response.data[i]['sum_score_predict']==null){
                response.data[i]['sum_score_predict'] = [];
              }
              response.data[i]['sum_score_predict'] = response1.data[i].hss_sum_score;
            }
            this.users = response.data;
        ...
        if(count_check==3){
          for (var index = 0; index < Object.keys(form.value).length; ++index) {
            ...
            if(h_a=='sum'){
              ...
              arrayData[hd_id]['sum'] = data_score;
            }
          }
          for (var index = 0; index < Object.keys(arrayData).length; ++index) {
            var hd_id  =  Object.keys(arrayData)[index];
            var data_sum_score = arrayData[hd_id]['sum'];
           // debugger;
            this.todoServcie.saveHandicapSumScore(this.member_id,hd_id,data_sum_score).subscribe((response)=>{
            // debugger;
            this.start_page_game_score();
            });
          }
