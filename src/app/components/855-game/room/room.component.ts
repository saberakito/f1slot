...
export class RoomComponentEight implements OnInit {
  ...
  percentLose:any;
  getPredict(id){
    this.TodoService.getPredict(id,'855_game',this.logic_id).subscribe(data => {
  ...
  getResutl(id){
    this.TodoService.getResultGame(id,'855_game',this.logic_id).subscribe(data => {
     console.log(data);
  ...
  getData(id){
    this.TodoService.getDataGameBacala(this.logic_id).subscribe(data => {
      ...
      var resultObject = this.search(id, data['855_game']);
