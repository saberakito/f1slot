...
export class RoomComponent implements OnInit {
  ...
  getPredict(id){
    this.TodoService.getPredict(id,'sagame').subscribe(data => {
      debugger;
     this.predict = data.result.p_predict;
     this.percent = data.result.percent;
     if(data.result.p_win_step==""||data.result.p_win_step==null){
  ...
  getResutl(id){
    this.TodoService.getResultGame(id,'sagame').subscribe(data => {
      //console.log(data);
  ...
  getData(id){
    ...
    var resultObject = this.search(id, data.sa_game);
