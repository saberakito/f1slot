...
export class RoomComponentDg implements OnInit {
  ...
  percentLose:any;
  getPredict(id){
    this.TodoService.getPredict(id,'dggame',this.logic_id).subscribe(data => {
  ...
  getResutl(id){
    this.TodoService.getResultGame(id,'dggame',this.logic_id).subscribe(data => {
     console.log(data);
  ...
  getData(id){
    this.TodoService.getDataGameBacala(this.logic_id).subscribe(data => {
      ...
      var resultObject = this.search(id, data['dg_game']);
